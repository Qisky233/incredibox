...
require ABSPATH.'/admin/admin-functions.php';

if(file_exists(ABSPATH . '/content/plugins/sitemap/admin-hook.php')){
    // Auto sitemap
    include ABSPATH . '/content/plugins/sitemap/admin-hook.php';
}

auto_add_games($cronjob_data);
	...
	if(is_null($check)){
		if($ref != 'upload'){
			...
			if(IMPORT_THUMB){
				...
				_import_thumbnail($_POST['thumb_2'], $slug, 2);
				...
				//
				_import_thumbnail($_POST['thumb_1'], $slug, 1);
				...
				if( SMALL_THUMB ){
					...
					if($use_webp){
						...
						_generate_small_thumbnail($_POST['thumb_2'], $slug);
					} else {
						_generate_small_thumbnail($_POST['thumb_2'], $slug);
					}
				}
...
function _generate_small_thumbnail($path, $slug){
...
function _import_thumbnail($url, $game_slug, $index = null){
	// copied from admin-functions.php
	$parent_dir = dirname(__FILE__) . '/../../../'; // CloudArcade root / installation folder
	if($url) {
		...
		if( get_setting_value('webp_thumbnail') ){
			...
			_save_remote_thumbnail($url, $new);
		}
	}
}
function _save_remote_thumbnail($source, $destination, $quality = 100) {
	// copied from admin-functions.php
	$ch = curl_init();
	// Set cURL options for retrieving the remote image file
